...
A reprodução recupera o padrão de [Baratto et al.\ 2022]: os interpoladores ficam
muito próximos e o IDW simples compete de igual para igual com o RF, que lidera por
margem desprezível. A contribuição acrescenta o rigor que faltava para discutir
isso: o \textbf{Wilcoxon} mostra que as diferenças, quando significativas, são
minúsculas; o \textbf{OK} dá o \emph{benchmark} geoestatístico; o \textbf{RFRK}
corrige o artefato ``em blocos'' do RF (com menor viés), ainda que sem ganho de
métrica; e a \textbf{importância de variáveis} revela que a habilidade do RF é,
sobretudo, \emph{sazonal}. A lição metodológica é que a escolha do ``melhor''
interpolador, neste problema, não se decide pelas métricas globais --- elas
empatam --- e sim por critérios práticos (suavidade do mapa, viés, custo).

\textbf{Limitações e desvios assumidos} (reprodução de \emph{metodologia}, não dos
números exatos): estações \emph{automáticas} do INMET (não as 11 convencionais),
outra região e período; ET\textsubscript{0} por Hargreaves--Samani (não
Penman--Monteith); covariáveis reduzidas a lon/lat/altitude (sem
declividade/aspecto/distância ao mar); ADW na forma canônica de New et al.\
($WE = w(1+a)$, enquanto o paper escreve $1-a$); e OK com variograma exponencial
fixo. \textbf{Trabalhos futuros}: ajuste de variograma por \emph{fold}, inclusão de
covariáveis topográficas (DEM), ET\textsubscript{0} por Penman--Monteith e a
inclusão explícita da sazonalidade nos interpoladores espaciais.
